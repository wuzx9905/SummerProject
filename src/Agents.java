...
public class Agents extends SimState {
    ...
    public static int agentsCount = 1000;
    public static int homesCount = 20;
    public static int meetingsCount = 20;
    public static int librariesCount = 20;
    
    public static double callRate = 0.05;
    
    //Location names
    public static String[] locations = {"home", "meeting","library","ER","CS","subway"};
    ...
    public void start(){
        ...
        for(int i = 0; i< agentsCount; i++){
            ...
            //define networks
            agent.familyCircle = (int)(i/ homesCount);
            agent.colleagueCircle = i % meetingsCount;
            //friend circle is random. To be updated.
            agent.friendCircle = (int)(random.nextDouble()* librariesCount);
            
            //Randomize call rate
            agent.conferenceRate = random.nextGaussian()*callRate/5+callRate;
            allAgents.add(agent);
        }
            ...
            public void step(SimState state){
                ...
                for(int i=0;i<agents.allAgents.size();i++){
                    ...
                    if (temp.isInvited){
                        ...
                        record = new RecordForLearning(temp.currentConference, agents);
                        if (temp.id==0)
                        ...
                        temp.isInvited = false;
                        temp.currentConference = null;
                    }
                }
                ...
                for(int i = 0; i<agents.conferencesInThisStep.size(); i++){
                    ...
                    for(int j = 0; j< conference.feedbacks.size(); j++){
                        feedback = (Feedback) conference.feedbacks.get(j);
                        if (feedback.payoff>0){
                            overallHappiness += 1.0/ conference.feedbacks.size();
                        }
                ...
                for(int i = 0; i<agents.conferencesInThisStep.size(); i++){
                    ...
                    multiplier = conference.feedbacks.size();
                    double n = 0.0;
                    for(int j = 0; j< conference.feedbacks.size(); j++){
                        n += ((Feedback) conference.feedbacks.get(j)).payoff;
                    }
                    if (multiplier>0)
                    	n /= multiplier;
                    payoff += n;
                ...
                if (out!=null){
                    try{
                        out.write(""+state.schedule.getSteps()+","+agents.conferencesInThisStep.size()+","+payoff+","+overallHappiness);
                    }catch(Exception e){
